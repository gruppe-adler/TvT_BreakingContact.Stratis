/*
	Author:
	Nelson Duarte, modified by nomisum
	...
	Examples:
	["Initialize", [player]] call GRAD_fnc_EGSpectator; 	// Initializes spectator for given player
	["Terminate"] call GRAD_fnc_EGSpectator; 			// Terminates spectator for given player
	["INERTIA"] call GRAD_fnc_EGSpectator; // toggles camera go smoooth
*/
...
// Name of this script
scriptName "GRAD_fnc_EGSpectator";
...
switch (_mode) do
{
	...
	case "Initialize" :
	{
		...
		if (!_bIsInitialized && _isPlayer) then
		{
			...
			["SetCameraInertia", [true]] call CAM;
			...
			{
				scriptName "GRAD_EGSpectator_thread";
	 ...
	 */
	 case "Inertia" :
	 {
		...
		if (["IsInitialized"] call SPEC) then
		{
		    ...
		    _inertia = missionNamespace getVariable [VAR_CAMERA_INERTIA, false];
		    ...
		    if (!isNull _camera) then
		    {
		        if (!_intertia) then
		        {
		            ...
		            missionNamespace setVariable [VAR_CAMERA_INERTIA, true];
		        }
		        else
		        {
		            _camera camCommand "inertia off"; // --> inertia off
		             missionNamespace setVariable [VAR_CAMERA_INERTIA, false];
		        };
		    };
	...
	case "GetTargetGroups" :
	{
		...
		{
			if ((_whitelistEmpty || { side _x in _whitelist }) && { {alive _x && {!(_x isKindOf SPECTATOR_CLASS)}} count units _x > 0 }) then
			{
	...
	case "CreateDisplay" :
	{
		...
		_display createDisplay "GRAD_RscDisplayEGSpectator";
	};
