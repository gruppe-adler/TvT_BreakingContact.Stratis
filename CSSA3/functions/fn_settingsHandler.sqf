...
switch (_action) do {
	case 'openSettings' : {
		...
		ctrlSetText [CSSA3_viewdistanceTextIDC, format ["%1 %2m", localize "STR_CSSA3_ViewDistance",round viewDistance]];
	...
	case 'distanceSliderChanged' : {
		if (_params select 1 > 0) then {
			...
			ctrlSetText [CSSA3_viewdistanceTextIDC, format ["%1 %2m", localize "STR_CSSA3_ViewDistance", viewDistance]];
		};
	};
