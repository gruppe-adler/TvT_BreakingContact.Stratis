...
testSpawnPositions = {
	...
	_testPos1 = [_center,[_spawnDistance,_spawnDistance + 10], random 360,0,[2,_searchDistance],[_searchDistance,"rhsusf_CH53E_USMC"]] call SHK_pos;
...
spawnBluforHQ = {
	...
	while {_waitingForBluforSpawn} do {
		...
		if (time > (_startTime + 30)) then {
			...
			[{hintSilent "no blufor spawn position found, adjusting search radius...";},"BIS_fnc_spawn",true,true] call BIS_fnc_MP;

			_bluforSearchDistance = _bluforSearchDistance + 50;
		};
		if (time > (_startTime + 60)) then {
			diag_log format ["fatal error : no blufor spawnpad position found. reducing spawn radius..."];
			[{hintSilent "no blufor spawn position found, adjusting search radius...";},"BIS_fnc_spawn",true,true] call BIS_fnc_MP;
			_bluforDistance = _bluforDistance - 500;
			_bluforSearchDistance = _bluforSearchDistance + 200;
		};
		...
		if (time > (_startTime + 300)) exitWith {
			...
			[{hintSilent "FATAL ERROR:  still no blufor spawnpad position found. please restart.";},"BIS_fnc_spawn",true,true] call BIS_fnc_MP;
		};
