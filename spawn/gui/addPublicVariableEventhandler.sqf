    ...
    if ((ISLAND_TARGET_POSITIONS select (ISLANDS find worldName)) select 3) then {
     ...
     USSupplies = [
        ...
        [
            ...
            (_this select 0) addItemCargoGlobal ['ACE_bloodIV_250',20];
            (_this select 0) addItemCargoGlobal ['ItemGPS',5];
            (_this select 0) setVariable ["detachableBoat",2];
            _box = createVehicle ['ACE_Box_Misc', position (_this select 0), [], 1, 'NONE'];
            _box = createVehicle ['ACE_Box_Misc', position (_this select 0), [], 1, 'NONE'];
            ['AddCargoByClass', _this select 0, ['ACE_wheel', _this select 0, 3]] call ace_common_fnc_targetEvent;
            ['AddCargoByClass', _this select 0, ['ACE_Track', _this select 0, 1]] call ace_common_fnc_targetEvent;
            },
            format['%1<br/>%2<br/>%3<br/>%4<br/>%5<br/>%6<br/>%7<br/>%8<br/>%9<br/>%10<br/>%11<br/>%12<br/>%13<br/>%14<br/>%15<br/>%16','Crew: 2','Seats: 11','Speed: 67','','Cargo:','4 AT M136','40 Smokes','60 AK Mags','20 MG Mags','20 Grenades','30 Flares','Medical Stuff','5 GPS','2 Assault Boats (ACE Interact)','3 Repair Wheels','1 Repair Track']
        ],
        [
            "B_Heli_Light_01_F",
            "MH-6",
