...
while {!GRAD_RADIO_VEH_MARKER_HIDDEN} do {
     if (_pulsesize > _pulseMaxSize) then {     	_pulsesize = 0.01;     	_modifier = 0.3;     	sleep _pulseDelayBetween;     };
     _pulsesize = _pulsesize + _modifier;     _modifier = _modifier + 0.1;     mrk_radioVeh setMarkerAlphaLocal 1 - (_pulsesize/_pulseMaxSize);     mrk_radioVeh setMarkerSizeLocal [_pulsesize, _pulsesize];          sleep _pulseSpeed;
