...
GRAD_tracking_mainLoop = [{
    ...
    if (_bothAreSending && !_finishedCloserThanUnfinished && _isCloseEnough) then {
        ...
        // check if distance changed, if yes, broadcast for client hint         [_modifier, _tempModifier, _tempDistance, _result] call GRAD_tracking_fnc_terminalCalculateDistanceModifier;     
    ...
    if (_terminalIsSending || _radioVehIsSending) then {
            ...
            grad_tracking_currentLoop = grad_tracking_currentLoop + 1;
             ...
             _radioVehX = getPos _radioVeh select 0;             _radioVehY = getPos _radioVeh select 1;             _markerPos = [_radioVehX, _radioVehY];
             if (!GRAD_TERMINAL_ACTIVE) then {                 _markerPos call GRAD_tracking_fnc_setRadioVehMarkerPosition;             } else {                 _markerPos call GRAD_tracking_fnc_setRadioVehMarkerPosition;                 [getPos _terminal select 0, getPos _terminal select 1] call GRAD_tracking_fnc_setTerminalMarkerPosition;             };         };
    };
